...
export class MyComment{
    ...
    public static of(myComment:MyComment[]){
        return myComment.map((comment)=>{
            return new MyComment(comment.commentId, comment.commentCreateAt, comment.content, comment.userTemplateFinishedAt,
                comment.writorNickname, comment.userTemplateId)
        })
    }
    ...
    setCommentCreateAt(commentCreateAt: Date): void {
        if(commentCreateAt === null)throw new InternalServerErrorException (`${__dirname} : commentCreateAt 값이 존재하지 않습니다.`);
    ...
    setUserTemplateFinishedAt(userTemplateFinishedAt: Date): void {
        if(userTemplateFinishedAt === null)throw new InternalServerErrorException (`${__dirname} : userTemplateFinishedAt 값이 존재하지 않습니다.`);
        this.userTemplateFinishedAt = userTemplateFinishedAt;
    }

    setWritorNickname(writorNickname: string): void {
        if(writorNickname === null)throw new InternalServerErrorException (`${__dirname} : writorNickname 값이 존재하지 않습니다.`);
