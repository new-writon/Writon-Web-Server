...
import { LikeRepository } from 'src/domain/template/application/port/output/Like.Repository';
...
export class LikeDao extends Repository<Likes> implements LikeRepository {
  ...
  async findLikeByAffiliationIdAndUserTemplateId(
    ...
    return this.dataSource
      ...
      .select('l')
      .from(Likes, 'l')
      .where('l.affiliation_id = :affiliationId', { affiliationId })
