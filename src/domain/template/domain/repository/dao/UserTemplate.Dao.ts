...
export class UserTemplateDao extends Repository<UserTemplate> implements UserTemplateRepository {
  ...
  async findUserTemplateAndCommentAndLikeByUserChallengeId(userChallengeId:number):Promise<UserTemplate[]>{
    return this.dataSource.createQueryBuilder(UserTemplate, 'ut')
      .leftJoinAndSelect('ut.comments', 'c')
      .leftJoinAndSelect('ut.likes', 'l')
      .where('ut.user_challenge_id = :userChallengeId',{userChallengeId})
      .getMany();
  }
  ...
  async findUserTemplateAndCommentAndLikeAndQeustionContentByUserChallengeIdAndDate(userChallengeId:number[], date:Date):Promise<UserTemplate[]>{
    return this.dataSource.createQueryBuilder(UserTemplate, 'ut')
      .leftJoinAndSelect('ut.comments', 'c')
      .leftJoinAndSelect('ut.likes', 'l', )
      .innerJoinAndSelect('ut.questionContents', 'qc', 'qc.visibility = 1')
  ...
  async findUserTemplateAndCommentAndLikeAndQeustionContentByUserChallengeId(userChallengeId:number):Promise<UserTemplate[]>{
    return this.dataSource.createQueryBuilder(UserTemplate, 'ut')
      .leftJoinAndSelect('ut.comments', 'c')
      .leftJoinAndSelect('ut.likes', 'l')
      .innerJoinAndSelect('ut.questionContents', 'qc')
      .where('ut.user_challenge_id = :userChallengeId',{userChallengeId})
      .getMany();
  }
  ...
  async findUserTemplateAndCommentAndLikeAndQeustionContentByUserTemplateIdWithVisibility(userTemplateId: number, visibility: boolean): Promise<UserTemplate> {
    return this.dataSource.createQueryBuilder()
        ...
        .leftJoinAndSelect('ut.comments', 'c')
        .leftJoinAndSelect('ut.likes', 'l')
        .innerJoinAndSelect('ut.questionContents', 'qc', 'qc.visibility = :visibility', { visibility})
