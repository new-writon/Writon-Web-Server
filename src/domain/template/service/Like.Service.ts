...
export class LikeServie {
  ...
  public async penetrateLike(
    ...
    const checkLikeData =
      await this.likeHelper.giveLikeByAffiliationIdAndUserTemplateId(
    ...
    const likeCount =
      await this.likeHelper.giveLikeCountByUserTemplateId(userTemplateId);
  ...
  public async bringLikeClickedUser(userTemplateId: number) {
    const likeDatas =
      await this.likeHelper.giveLikesByUserTemplateId(userTemplateId);
  ...
  private async processLikeClickedLogic(likeDatas: Likes[]) {
    const extractedAffiliationIds =
      this.dataMapperService.extractAffiliationId(likeDatas);
