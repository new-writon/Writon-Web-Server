...
export class ChallengeInformationService {
  ...
  public async checkChallengeDay(challengeId: number, date: string) {
    const challengeDayData =
      await this.challengeDayHelper.giveChallengeDayByChallengeIdAndDate(
        challengeId,
        date,
      );
  ...
  public async bringChallengeAccordingToOrganization() {
    ...
    const challengeDatas =
      await this.challengeHelper.giveChallengeByOrgnizationIds(
        extractedOrganizationIds,
      );
    const mappedChallengeAndOrganization = this.mappingChallengeAndOrganization(
      organizationDatas,
      challengeDatas,
    );
    const sortedallChallengeAccordingToOrganizationData =
      this.sortChallengePerOrganization(mappedChallengeAndOrganization);
    return ChallengeAccordingToOrganization.of(
      sortedallChallengeAccordingToOrganizationData,
    );
  }

  public async bringChallengeDay(challengeId: number): Promise<string[]> {
    const challengeDay =
      await this.challengeDayHelper.giveChallengeDayByChallengeId(challengeId);
  ...
  public mappingChallengeAndOrganization(
    ...
    return organizations.flatMap((organization) => {
      ...
      return relatedChallenges.map((data) => {
        return new ChallengeAndOrganization(
  ...
  private sortChallengePerOrganization(
    ...
    const sortData: ChallengeAccordingToOrganization[] = Object.entries(
      groupOrganization,
    ).map(([organization, challenges]) => {
